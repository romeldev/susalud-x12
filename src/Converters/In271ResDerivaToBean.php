...
class In271ResDerivaToBean
{
    /**
     * @param string $cadena
     * @return In271ResDeriva
     */
    public static function traducirEstructura271ResDeriva($cadena)
    {
        $flagISA = true;
        $flagGS = true;
        $flagST = true;
        ...
        foreach ($arrayCadena as $dataCadena) {
            ...
            if ($segmentId === 'ISA') {
                if (!$flagISA) { continue; }
                $bean->noTransaccion = '271_RES_DERIVA';
                ...
                if ($tagHL === '1') {
                    ...
                    if ($tagNM1 === 1) {
                        $detalle = new In271ResDerivaDetalle();
                        ...
                        $last->noLuAtencion = isset($s1[3]) ? $s1[3] : '';
                        $tagNM1 = 1;
                    }
                }
                ...
                if ($last) {
                    $last->feAtSalud = isset($s1[3]) ? $s1[3] : '';
                }
                ...
                if ($tagREF === 1 && $last) {
                    ...
                    $last->tiDoTrabajoAfiliado = isset($s1[2]) ? $s1[2] : '';
                    if (isset($s1[4])) {
                        $ref04 = explode(':', $s1[4]);
                        $last->nuDoTrabajoAfiliado = isset($ref04[1]) ? $ref04[1] : '';
                    }
                    $tagREF++;
                } elseif ($tagREF === 4 && $last) {
                    $last->nuAtencion = isset($s1[2]) ? $s1[2] : '';
                    $last->teMsgLibre1 = isset($s1[3]) ? $s1[3] : '';
                    $tagREF++;
                } elseif ($tagREF === 5 && $last) {
                    $last->coTiProducto = isset($s1[2]) ? $s1[2] : '';
                    $last->deProducto = isset($s1[3]) ? $s1[3] : '';
                    $tagREF++;
                    ...
                    if (isset($s1[4])) {
                        $ref04 = explode(':', $s1[4]);
                        $last->idCaReferencia = isset($ref04[0]) ? $ref04[0] : '';
